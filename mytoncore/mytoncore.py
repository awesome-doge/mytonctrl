...
class MyTonCore():
	...
	def get_destination_addr(self, destination):
		...
		if bookmarks:
			for bookmark in bookmarks:
				if destination == bookmark.get("name"):
					return bookmark.get("addr")
		# check if it is address
		if self.IsAddr(destination):
			return destination
		raise Exception(f"get_destination_addr error: unknown destination: {destination}")
	# end define

	def AddrFull2AddrB64(self, addrFull, bounceable=True):
		# Get buffer
		bname = "addrFull2AddrB64"
		buff = self.GetFunctionBuffer(bname, timeout=60)
		if buff:
			return buff
		#end if

		self.local.add_log("start AddrFull2AddrB64 function", "debug")
		result = self.liteClient.Run(f"addr2b64 {addrFull}")
		addrB64 = parse(result, "got address: ", "\n")
		if bounceable:
			return addrB64
		else:
			return addrB64 + "?tx=0"
	...
	def IsAddr(self, addr):
		...
		self.local.add_log("start IsAddr function", "debug")
		result = self.liteClient.Run(f"isaddr {addr}")
		return parse(result, "isaddr: ", "\n") == "true"
	#end define

	def GetSettings(self, name):
		# self.local.load_db()
		data = self.local.db.get(name)
		return data
	#end define
	...
	def migrate_to_modes(self):
		if self.local.db.get("validator") is not None:
			self.enable_mode("validator")
	# ... existing code ...